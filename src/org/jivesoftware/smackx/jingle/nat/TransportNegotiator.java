...
public abstract class TransportNegotiator extends JingleNegotiator {
	...
	public static final class Ice extends TransportNegotiator {
		...
		public TransportCandidate getBestRemoteCandidate() {
			...
			final List<TransportCandidate> cands = getValidRemoteCandidatesList();
			if (!cands.isEmpty()) {
				int highest = -1;
				ICECandidate chose = null;
				for (final TransportCandidate transportCandidate : cands) {
					if (((ICECandidate) transportCandidate).getPreference() > highest) {
						chose = (ICECandidate) transportCandidate;
						highest = ((ICECandidate) transportCandidate)
								.getPreference();
					}
				}
				result = chose;
	...
	public static final class RawUdp extends TransportNegotiator {
		...
		public TransportCandidate getBestRemoteCandidate() {
			...
			final ArrayList<TransportCandidate> cands = getValidRemoteCandidatesList();
			if (!cands.isEmpty()) {
