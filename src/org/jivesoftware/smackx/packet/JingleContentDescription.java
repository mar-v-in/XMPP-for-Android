...
public abstract class JingleContentDescription implements PacketExtension {
	 ...
	 */
	public ArrayList<PayloadType> getAudioPayloadTypesList() {
		final ArrayList<PayloadType> result = new ArrayList<PayloadType>();
		final Iterator<JinglePayloadType> jinglePtsIter = getJinglePayloadTypes();

		while (jinglePtsIter.hasNext()) {
			final JinglePayloadType jpt = jinglePtsIter.next();
	...
	public String toXML() {
		...
		synchronized (payloads) {
			if (payloads.size() > 0) {
				...
				final Iterator<JinglePayloadType> pt = payloads.listIterator();
				while (pt.hasNext()) {
					final JinglePayloadType pte = pt.next();
