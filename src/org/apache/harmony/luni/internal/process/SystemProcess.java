...
public class SystemProcess extends Process {
	...
	public static Process create(String[] progArray, String[] envp,
			...
			public void run() {
				...
				try {
					procVals = createImpl(
							...
							progBytes,
							envBytes,
							directory == null ? null : Util.getBytes(directory
									.getPath()));
				...
				p.handle = procVals[0];
				p.in = new BufferedOutputStream(new ProcessOutputStream(
						procVals[1]));
				p.out = new BufferedInputStream(new ProcessInputStream(
						procVals[2]));
				p.err = new BufferedInputStream(new ProcessInputStream(
						procVals[3]));
		...
		final Thread wait = new Thread(waitingThread);
		wait.setDaemon(true);
		wait.start();
	...
	protected synchronized static native long[] createImpl(Process p,
			byte[][] progArray, byte[][] envp, byte[] dir) throws IOException;
