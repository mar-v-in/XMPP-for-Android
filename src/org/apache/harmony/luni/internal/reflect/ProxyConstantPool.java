...
class ProxyConstantPool implements ProxyConstants {
	...
	int literalIndex(char[] utf8Constant) {
		...
		if ((index = UTF8Cache.get(utf8Constant)) < 0) {
			writeU1(Utf8Tag);
			final int savedCurrentOffset = currentOffset;
			...
			currentOffset += 2;
			int length = 0;
			for (final char current : utf8Constant) {
	...
	int literalIndex(Constructor<?> aMethod) {
		...
		if ((index = methodCache.get(aMethod)) < 0) {
			final int classIndex = typeIndex(aMethod.getDeclaringClass()
					.getName());
			final int nameAndTypeIndex = literalIndexForNameAndType(
					literalIndex(Init),
	...
	int literalIndex(Field aField) {
		return literalIndex(aField.getDeclaringClass().getName(),
	...
	int literalIndex(String declaringClass, String name, Class clazz) {
		int index;
		final String key = declaringClass + "." + name;
		if ((index = fieldCache.get(key)) < 0) {
			final int classIndex = typeIndex(declaringClass);
			final int nameAndTypeIndex = literalIndexForNameAndType(
	...
	int literalIndexForLdc(char[] stringCharArray) {
		...
		if ((index = stringCache.get(stringCharArray)) < 0) {
			...
			if ((stringIndex = UTF8Cache.get(stringCharArray)) < 0) {
				...
				currentOffset += 2;
				int length = 0;
				for (final char current : stringCharArray) {
					if ((current >= 0x0001) && (current <= 0x007F)) {
						// we only need one byte: ASCII table
						writeU1(current);
						length++;
	...
	int literalIndexForNameAndType(int nameIndex, int typeIndex) {
		int index;
		final int[] key = new int[] { nameIndex, typeIndex };
	...
	int typeIndex(String typeName) {
		...
		final char[] charArray = typeName.toCharArray();
		if ((index = classNameCache.get(charArray)) < 0) {
			final int nameIndex = literalIndex(charArray);
