...
class ProxyCharArrayCache {
	...
	ProxyCharArrayCache(int initialCapacity) {
		...
		elementSize = 0;
		threshold = (int) (initialCapacity * 0.66f);
		keyTable = new char[initialCapacity][];
	...
	private int hashCodeChar(char[] val) {
		...
		int hash = 0;
		final int n = 2; // number of characters skipped
	...
	private void rehash() {
		final ProxyCharArrayCache newHashtable = new ProxyCharArrayCache(
				keyTable.length * 2);
		...
		keyTable = newHashtable.keyTable;
		valueTable = newHashtable.valueTable;
		threshold = newHashtable.threshold;
	}
