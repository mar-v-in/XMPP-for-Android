...
class ProxyMethod {
	...
	ProxyMethod(Class declaringClass, Method method) {
		...
		this.method = method;
		commonExceptions = method.getExceptionTypes();
	}
	...
	Class[] getCheckedExceptions() {
		final Class[] newExceptions = commonExceptions.clone();
		int cLength = newExceptions.length;
		for (int c = 0, cL = cLength; c < cL; c++) {
			final Class<?> ex = newExceptions[c];
		...
		final Class[] result = new Class[cLength + 2];
		int index = 0;
		result[index++] = Error.class;
		result[index++] = RuntimeException.class;
		for (final Class<?> ex : newExceptions) {
			if (ex != null) {
				result[index++] = ex;
			}
		}
	...
	boolean matchMethod(Method otherMethod) {
		...
		final Class[] params2 = otherMethod.getParameterTypes();
		...
		final Class<?> thisMethodReturnType = method.getReturnType();
		final Class<?> otherMethodReturnType = otherMethod.getReturnType();
		if (!thisMethodReturnType.isAssignableFrom(otherMethodReturnType)) {
			if (otherMethodReturnType.isAssignableFrom(thisMethodReturnType)) {
				...
				throw new IllegalArgumentException(Messages.getString(
						"luni.19", method.getName()));
			}
		}

		if (commonExceptions.length != 0) {
			final Class[] otherExceptions = otherMethod.getExceptionTypes();
			if (otherExceptions.length == 0) {
				...
				nextException: for (int c = 0, cL = cLength, oL = otherExceptions.length; c < cL; c++) {
					...
					for (int o = 0; o < oL; o++) {
						final Class<?> oException = otherExceptions[o];
						...
						if (cException.isAssignableFrom(oException)) {
							...
							if (!containsClass(commonExceptions, oException)) {
								// if exceptions in throw list have Parent-Child
								// relationship just ignore it
				...
				if (cLength != commonExceptions.length) {
					final Class[] newExceptions = new Class[cLength];
					for (int i = 0, j = 0, length = commonExceptions.length; i < length; i++) {
						final Class<?> ex = commonExceptions[i];
						if (ex != null) {
							newExceptions[j++] = ex;
						}
					}
