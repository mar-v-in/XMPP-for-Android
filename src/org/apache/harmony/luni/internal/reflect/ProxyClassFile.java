...
public final class ProxyClassFile implements ProxyConstants {
	...
	public static byte[] generateBytes(String typeName, Class[] interfaces) {
		final ProxyClassFile classFile = new ProxyClassFile(typeName,
				interfaces);
	...
	static char[] getConstantPoolName(Constructor<?> method) /*
		...
		final StringBuilder buffer = new StringBuilder(
				parameters.length + 1 * 20);
		buffer.append('(');
		for (final Class<?> element : parameters) {
	...
	private final ProxyConstantPool constantPool;
	...
	ProxyClassFile(String typeName, Class[] interfaces) {
		...
		final int accessFlags = AccPublic | AccFinal | AccSuper;
		contents[contentsOffset++] = (byte) (accessFlags >> 8);
		contents[contentsOffset++] = (byte) accessFlags;
		final int classNameIndex = constantPool.typeIndex(typeName);
		contents[contentsOffset++] = (byte) (classNameIndex >> 8);
		contents[contentsOffset++] = (byte) classNameIndex;
		final int superclassNameIndex = constantPool
				.typeIndex("java/lang/reflect/Proxy");
		contents[contentsOffset++] = (byte) (superclassNameIndex >> 8);
		contents[contentsOffset++] = (byte) superclassNameIndex;
		final int interfacesCount = interfaces.length;
		contents[contentsOffset++] = (byte) (interfacesCount >> 8);
		contents[contentsOffset++] = (byte) interfacesCount;
		for (int i = 0; i < interfacesCount; i++) {
			final int interfaceIndex = constantPool.typeIndex(interfaces[i]
					.getName());
	...
	private void addAttributes() {
		...
		final int constantPoolCount = constantPool.currentIndex;
	...
	private void addFields() {
		...
		final int methodCount = proxyMethods.length;
		writeUnsignedShort(methodCount);
		for (int i = 0; i < methodCount; i++) {
			writeUnsignedShort(AccPublic | AccStatic);
			final String methodName = getFieldNamebyMethodName(
					proxyMethods[i].method.getName(), i);
			writeUnsignedShort(constantPool.literalIndex(methodName
					.toCharArray()));
			writeUnsignedShort(constantPool
					.literalIndex("Ljava/lang/reflect/Method;".toCharArray()));
	...
	private void addMethods() {
		...
		for (int i = 0; i < methodCount; i++) {
			final ProxyMethod pMethod = proxyMethods[i];
			final Method method = pMethod.method;
			...
			final Class[] thrownsExceptions = pMethod.commonExceptions;
			final int eLength = thrownsExceptions.length;
			if (eLength > 0) {
	...
	private void completeCodeDescription(int codeLength, int offset,
			int maxLocals) {
	...
	private void findMethods(Class[] interfaces) {
		...
		final ArrayList<ProxyMethod> allMethods = new ArrayList<ProxyMethod>(25);
		...
		final HashSet<Class<?>> interfacesSeen = new HashSet<Class<?>>();
	...
	private void findMethods(Class<?> nextInterface,
		...
		final int existingMethodCount = allMethods.size();
		final Method[] methods = nextInterface.getMethods();
		...
		final Class<?>[] superInterfaces = nextInterface.getInterfaces();
		for (final Class<?> element : superInterfaces) {
	...
	private void generateCodeAttribute(ProxyMethod pMethod, int index) {
		final int codeAttributeOffset = contentsOffset;
		 ...
		 * invokeinterface 67
		 * java.lang.reflect.InvocationHandler.invoke(Ljava.lang
		 * .Object;Ljava.lang
		 * .reflect.Method;[Ljava.lang.Object;)Ljava.lang.Object; cast return
		 * result catch & convert exceptions if necessary
		 */

		final int codeStartOffset = contentsOffset;
		writeUnsignedByte(OPC_aload_0);
		...
		writeUnsignedShort(constantPool.literalIndex(typeName,
				getFieldNamebyMethodName(pMethod.method.getName(), index),
				Method.class));
		...
		final Class[] checkedExceptions = pMethod.getCheckedExceptions();
		final int checkedLength = checkedExceptions.length;
		if (checkedLength > 0) {
			final int codeEndIndex = contentsOffset - codeStartOffset;
		 ...
		 */
		final int codeAttributeLength = contentsOffset
				- (codeAttributeOffset + 6);
		...
		final int maxStack = maxLocals + 10; // larger than the exact amount
	...
	private void generateCodeAttribute3() {
		writeUnsignedShort(constantPool.literalIndex(CodeName));
		final int codeStart = contentsOffset;
		...
		final int targetForGotoOffset = contentsOffset;
		writeUnsignedShort(4); // to be updated
		final int handlerStart = contentsOffset - codeStart - 12;
	...
	private void generateExceptionHandler() {
		...
		writeUnsignedByte(OPC_getstatic);
		writeUnsignedShort(constantPool.literalIndex("java/lang/System", "err",
				PrintStream.class));
		Method m = null;
		try {
			m = Exception.class.getMethod("printStackTrace",
	...
	private byte[] getBytes() {
		final byte[] fullContents = new byte[headerOffset + contentsOffset];
	...
	private void initializeField(ProxyMethod m, int index) {
		final String methodName = m.method.getName();
		final Class[] params = m.method.getParameterTypes();
		final int paramLengths = params.length;
		...
		for (int i = 0; i < paramLengths; i++) {
			...
			if (params[i].isPrimitive()) {
				writeUnsignedByte(OPC_getstatic);
				writeUnsignedShort(constantPool.literalIndex(
						typeWrapperName(params[i]), "TYPE", Class.class));
		...
		writeUnsignedShort(constantPool.literalIndex(typeName,
				getFieldNamebyMethodName(methodName, index), Method.class));
	}
	...
	private void writeLdc(String constant) {
		final int index = constantPool.literalIndexForLdc(constant
				.toCharArray());
		if (index <= 0) {
			throw new InternalError();
		}
	...
	private void writeLdcWithClass(String name) {
		final int index = constantPool.typeIndex(name);
		if (index <= 0) {
			throw new InternalError();
		}
		if (index > 255) {
	...
	private void writeStaticInitializer() {
		...
		writeUnsignedShort(constantPool.literalIndex(new char[] { '<', 'c',
				'l', 'i', 'n', 'i', 't', '>' }));
		writeUnsignedShort(constantPool
				.literalIndex(new char[] { '(', ')', 'V' }));
	...
	private void writeUnsignedByte(int b) {
		try {
			...
			final int actualLength = contents.length;
