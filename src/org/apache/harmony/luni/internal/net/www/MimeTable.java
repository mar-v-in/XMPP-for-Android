...
public class MimeTable implements FileNameMap {
	 ...
	 */
	public MimeTable() {
		final InputStream str = AccessController
	...
	public String getContentTypeFor(String filename) {
		...
		final int firstCharInExtension = filename.lastIndexOf('.') + 1;
	...
	private InputStream getContentTypes() {
		// User override?
		final String userTable = System.getProperty("content.types.user.table"); //$NON-NLS-1$
		if (userTable != null) {
			try {
		...
		final File contentFile = new File(javahome, "lib" //$NON-NLS-1$
