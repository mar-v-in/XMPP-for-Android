...
public class Header implements Cloneable {
	...
	public Header() {
		super();
		props = new ArrayList<String>(20);
		keyTable = new TreeMap<String, LinkedList<String>>(
	...
	public Header(Map<String, List<String>> map) {
		...
		for (final Entry<String, List<String>> next : map.entrySet()) {
	...
	public Object clone() {
		try {
			...
			clone.keyTable = new TreeMap<String, LinkedList<String>>(
					String.CASE_INSENSITIVE_ORDER);
			for (final Map.Entry<String, LinkedList<String>> next : keyTable
					.entrySet()) {
				final LinkedList<String> v = (LinkedList<String>) next
						.getValue().clone();
	...
	public String get(String key) {
		final LinkedList<String> result = keyTable.get(key);
		if (result == null) {
			return null;
		}
	...
	public Map<String, List<String>> getFieldMap() {
		...
		for (final Map.Entry<String, LinkedList<String>> next : keyTable
				.entrySet()) {
			final List<String> v = next.getValue();
	...
	public void set(String key, String value) {
		...
		final LinkedList<String> list = keyTable.get(key);
		if (list == null) {
			add(key, value);
		} else {
			list.clear();
			...
			for (int i = 0; i < props.size(); i += 2) {
				final String propKey = props.get(i);
