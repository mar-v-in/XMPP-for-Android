...
public class HttpURLConnectionImpl extends HttpURLConnection {
	private class ChunkedInputStream extends InputStream {
		...
		public int read(byte[] buf, int offset, int length) throws IOException {
			...
			final int result = is.read(buf, offset, length);
			if (result > 0) {
		...
		private void readChunkSize() throws IOException {
			...
			String size = readln();
			final int index = size.indexOf(";"); //$NON-NLS-1$
			if (index >= 0) {
	...
	private class HttpOutputStream extends OutputStream {
		...
		private int calculateChunkDataLength() {
			 ...
			 */
			final int bitSize = Integer.toHexString(defaultCacheSize).length();
			/*
			 * here is the calculated head size, not real size (for 19, it
			 * counts 3, not real size 4)
			 */
			final int headSize = (Integer.toHexString(defaultCacheSize
					- bitSize - 2).length()) + 2;
		...
		public synchronized void write(byte[] buffer, int offset, int count)
			...
			if (offset < 0 || count < 0 || offset > buffer.length
				...
				throw new ArrayIndexOutOfBoundsException(
						Messages.getString("luni.13")); //$NON-NLS-1$
			}
			...
			if (limit >= 0) {
				...
				if (!writeToSocket || cache.size() + count < cacheLength) {
					...
					final int writeNum = cacheLength - cache.size();
	...
	public void connect() throws IOException {
		...
		if (proxy != null) {
			...
			final List<Proxy> proxyList = selector.select(uri);
			if (proxyList != null) {
				for (final Proxy selectedProxy : proxyList) {
	...
	private byte[] createRequest() throws IOException {
		...
		for (int i = 0; i < reqHeader.length(); i++) {
			final String key = reqHeader.getKey(i);
			if (key != null) {
		...
		if (reqHeader.get("User-Agent") == null) { //$NON-NLS-1$
			...
			final String agent = getSystemProperty("http.agent"); //$NON-NLS-1$
			if (agent == null) {
	...
	void doRequestInternal() throws IOException {
		...
		while (true) {
			...
			if (responseCode == HTTP_PROXY_AUTH) {
				...
				// username/password
				// until authorized
				final String challenge = resHeader.get("Proxy-Authenticate"); //$NON-NLS-1$
				if (challenge == null) {
				...
				disconnect();
				connected = false;
				final String credentials = getAuthorizationCredentials(challenge);
				if (credentials == null) {
			...
			if (responseCode == HTTP_UNAUTHORIZED) {
				...
				final String challenge = resHeader.get("WWW-Authenticate"); //$NON-NLS-1$
				if (challenge == null) {
					break;
				}
				// drop everything and reconnect, might not be required for
				...
				if ((responseCode == HTTP_MULT_CHOICE
					...
					if (++redirect > 4) {
						throw new ProtocolException(
								Messages.getString("luni.2F")); //$NON-NLS-1$
					}
					final String location = getHeaderField("Location"); //$NON-NLS-1$
					if (location != null) {
						// start over
	...
	private String getAuthorizationCredentials(String challenge)
		...
		final int idx = challenge.indexOf(" "); //$NON-NLS-1$
		final String scheme = challenge.substring(0, idx);
		final int realm = challenge.indexOf("realm=\"") + 7; //$NON-NLS-1$
		String prompt = null;
		if (realm != -1) {
			final int end = challenge.indexOf('"', realm);
			if (end != -1) {
		...
		// the password
		final PasswordAuthentication pa = Authenticator
		...
		final byte[] bytes = (pa.getUserName() + ":" + new String(pa.getPassword())) //$NON-NLS-1$
				.getBytes("ISO8859_1"); //$NON-NLS-1$
		final String encoded = Base64.encode(bytes, "ISO8859_1"); //$NON-NLS-1$
	...
	private InputStream getContentStream() throws IOException {
		...
		final String encoding = resHeader.get("Transfer-Encoding"); //$NON-NLS-1$
		if (encoding != null && encoding.toLowerCase().equals("chunked")) { //$NON-NLS-1$
			return uis = new ChunkedInputStream();
		}

		final String sLength = resHeader.get("Content-Length"); //$NON-NLS-1$
		if (sLength != null) {
			try {
	...
	private boolean getFromCache() throws IOException {
		if (useCaches && null != responseCache && !hasTriedCache) {
			...
			if (null != cacheResponse) {
				final Map<String, List<String>> headMap = cacheResponse
						.getHeaders();
				if (null != headMap) {
	...
	public OutputStream getOutputStream() throws IOException {
		...
		int limit = -1;
		final String contentLength = reqHeader.get("Content-Length"); //$NON-NLS-1$
	...
	public void setIfModifiedSince(long newValue) {
		...
		final String date = sdf.format(new Date(newValue));
	...
	private void setProxy(String proxy) {
		...
		if (index == -1) {
			...
			final String port = proxy.substring(index + 1);
			try {
				hostPort = Integer.parseInt(port);
			} catch (final NumberFormatException e) {
				throw new IllegalArgumentException(Messages.getString(
						"luni.30", port)); //$NON-NLS-1$
			}
			if (hostPort < 0 || hostPort > 65535) {
				throw new IllegalArgumentException(
						Messages.getString("luni.31")); //$NON-NLS-1$
			}
		}
	}
