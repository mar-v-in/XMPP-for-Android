...
public class PlainDatagramSocketImpl extends DatagramSocketImpl {
	...
	private final boolean bindToDevice = false;

	private byte[] ipaddress = { 0, 0, 0, 0 };

	private int ttl = 1;

	private final INetworkSystem netImpl = Platform.getNetworkSystem();
	...
	public void bind(int port, InetAddress addr) throws SocketException {
		final String prop = AccessController
				.doPrivileged(new PriviAction<String>("bindToDevice")); //$NON-NLS-1$
	...
	public int getTimeToLive() throws IOException {
		...
		// closed.
		final int result = (((Byte) getOption(IP_MULTICAST_TTL)).byteValue()) & 0xFF;
	...
	public byte getTTL() throws IOException {
		// Call the native first so there will be an exception if the socket if
		// closed.
		final byte result = ((Byte) getOption(IP_MULTICAST_TTL)).byteValue();
		if ((netImpl.getSocketFlags() & MULTICAST_TTL) != 0) {
			return (byte) ttl;
		}
		return result;
	...
	public void joinGroup(SocketAddress addr, NetworkInterface netInterface)
		...
		if (addr instanceof InetSocketAddress) {
			final InetAddress groupAddr = ((InetSocketAddress) addr)
					.getAddress();
	...
	protected int peek(InetAddress sender) throws IOException {
		if (isNativeConnected) {
			 ...
			 */
			final byte[] storageArray = new byte[10];
			final DatagramPacket pack = new DatagramPacket(storageArray,
					storageArray.length);
	...
	public void setOption(int optID, Object val) throws SocketException {
		 ...
		 * REUSEADDR AND REUSEPORT so for REUSEADDR set this option which tells
		 * the VM to set the two values as appropriate for the platform
		 */
		...
		if (optID == SocketOptions.SO_TIMEOUT) {
			...
			final int flags = netImpl.getSocketFlags();
			try {
			...
			if (optID == SocketOptions.IP_MULTICAST_IF
				...
				final InetAddress inet = (InetAddress) val;
