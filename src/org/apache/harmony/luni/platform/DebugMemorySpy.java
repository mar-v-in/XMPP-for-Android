...
final class DebugMemorySpy extends AbstractMemorySpy {
	...
	public boolean free(PlatformAddress address) {
		final boolean vetoed = super.free(address);
		if (!vetoed && stackDump) {
			Thread.dumpStack();
		}
		return vetoed;
	 ...
	 * quite possible that the range checker does <em>not</em> catch some buffer
	 * overruns.
	 */
	@Override
	public void rangeCheck(PlatformAddress address, int offset, int length)
			throws IndexOutOfBoundsException {
		final AddressWrapper wrapper = memoryInUse.get(address);
		if (wrapper != null) {
			final PlatformAddress accessStart = address.offsetBytes(offset);
			final PlatformAddress accessEnd = accessStart.offsetBytes(length);
			final PlatformAddress allocStart = wrapper.shadow;
			final PlatformAddress allocEnd = allocStart
					.offsetBytes(address.size);
			final boolean under = (accessStart.compareTo(allocStart)) == -1;
			final boolean over = (accessEnd.compareTo(allocEnd)) == 1;
			if (under || over) {
	...
	private void startSpy() {
		final Thread spy = new Thread(new Runnable() {
			@Override
			public void run() {
				while (true) {
					try {
