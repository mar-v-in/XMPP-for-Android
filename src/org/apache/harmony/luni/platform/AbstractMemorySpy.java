...
abstract class AbstractMemorySpy implements IMemorySpy {
	...
	final class AddressWrapper {
		...
		AddressWrapper(PlatformAddress address) {
			super();
			shadow = address.duplicate();
			wrAddress = new PhantomReference<PlatformAddress>(address,
					notifyQueue);
		}
	}

	// TODO: figure out how to prevent this being a synchronization bottleneck
	protected Map<PlatformAddress, AddressWrapper> memoryInUse = new HashMap<PlatformAddress, AddressWrapper>(); // Shadow
																													// to
																													// Wrapper

	protected Map<Reference, PlatformAddress> refToShadow = new HashMap<Reference, PlatformAddress>(); // Reference
																										// to
																										// Shadow
	...
	public void alloc(PlatformAddress address) {
		final AddressWrapper wrapper = new AddressWrapper(address);
		synchronized (this) {
	 ...
	 * 
	 * @see
	 * org.apache.harmony.luni.platform.struct.IMemorySpy#autoFree(org.apache
	 * .harmony.luni.platform.struct.PlatformAddress)
	 */
	@Override
	...
	protected void orphanedMemory(Reference ref) {
		AddressWrapper wrapper;
		synchronized (this) {
			final PlatformAddress shadow = refToShadow.remove(ref);
