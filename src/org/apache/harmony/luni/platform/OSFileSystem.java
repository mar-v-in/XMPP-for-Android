...
class OSFileSystem implements IFileSystem {
	...
	public long available(long fileDescriptor) throws IOException {
		final long nChar = availableImpl(fileDescriptor);
		if (nChar < 0) {
	...
	public void close(long fileDescriptor) throws IOException {
		final int rc = closeImpl(fileDescriptor);
		if (rc == -1) {
			throw new IOException();
		}
	}
	...
	public void fflush(long fileDescriptor, boolean metadata)
			throws IOException {
		final int result = fflushImpl(fileDescriptor, metadata);
		if (result == -1) {
			throw new IOException();
		}
	}
	...
	public boolean lock(long fileDescriptor, long start, long length, int type,
		...
		final int result = lockImpl(fileDescriptor, start, length, type,
				waitFlag);
		return result != -1;
	}
	...
	public long open(byte[] fileName, int mode) throws FileNotFoundException {
		...
		final long handler = openImpl(fileName, mode);
		if (handler < 0) {
			try {
	...
	public long read(long fileDescriptor, byte[] bytes, int offset, int length)
		...
		final long bytesRead = readImpl(fileDescriptor, bytes, offset, length);
		if (bytesRead < -1) {
			/*
			 * TODO: bytesRead is never less than -1 so this code does nothing?
			 * The native code throws an exception in only one case so perhaps
			 * this should be 'bytesRead < 0' to handle any other cases. But the
			 * other cases have been ignored until now so fixing this could
			 * break things
			 */
			throw new IOException();
		}
	...
	public long readDirect(long fileDescriptor, long address, int offset,
		...
		final long bytesRead = readDirectImpl(fileDescriptor, address, offset,
				length);
		if (bytesRead < -1) {
			throw new IOException();
		}
	...
	public long readv(long fileDescriptor, long[] addresses, int[] offsets,
		...
		final long bytesRead = readvImpl(fileDescriptor, addresses, offsets,
				lengths, size);
		if (bytesRead < -1) {
			throw new IOException();
		}
	...
	public long seek(long fileDescriptor, long offset, int whence)
			throws IOException {
		final long pos = seekImpl(fileDescriptor, offset, whence);
		if (pos == -1) {
			throw new IOException();
		}
		return pos;
	...
	public long size(long fileDescriptor) throws IOException {
		final long nChar = sizeImpl(fileDescriptor);
		if (nChar < 0) {
			throw new IOException();
		}
		return nChar;
	...
	public long transfer(long fileHandler, FileDescriptor socketDescriptor,
		...
		final long result = transferImpl(fileHandler, socketDescriptor, offset,
				count);
		if (result < 0) {
			throw new IOException();
		}
	...
	public void truncate(long fileDescriptor, long size) throws IOException {
		final int rc = truncateImpl(fileDescriptor, size);
		if (rc < 0) {
			throw new IOException();
		}
	}
	...
	public long ttyAvailable() throws IOException {
		final long nChar = ttyAvailableImpl();
		if (nChar < 0) {
			throw new IOException();
		}
		return nChar;
	...
	public long ttyRead(byte[] bytes, int offset, int length)
			throws IOException {
		final long nChar = ttyReadImpl(bytes, offset, length);
		if (nChar < 0) {
			throw new IOException();
		}
		return nChar;
	...
	public void unlock(long fileDescriptor, long start, long length)
		...
		final int result = unlockImpl(fileDescriptor, start, length);
		if (result == -1) {
			throw new IOException();
		}
	}
	...
	public long write(long fileDescriptor, byte[] bytes, int offset, int length)
			throws IOException {
		final long bytesWritten = writeImpl(fileDescriptor, bytes, offset,
				length);
		if (bytesWritten < 0) {
			throw new IOException();
		}
	...
	public long writeDirect(long fileDescriptor, long address, int offset,
			int length) throws IOException {
		final long bytesWritten = writeDirectImpl(fileDescriptor, address,
				offset, length);
		if (bytesWritten < 0) {
			throw new IOException();
		}
