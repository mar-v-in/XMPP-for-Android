...
public class PlatformAddressFactory {
	...
	public static PlatformAddress alloc(long size) {
		final long osAddress = PlatformAddress.osMemory.malloc(size);
		final PlatformAddress newMemory = on(osAddress, size);
	...
	public static PlatformAddress allocMap(long fd, long start, long size,
		...
		final long osAddress = PlatformAddress.osMemory.mmap(fd, start, size,
				mode);
		final PlatformAddress newMemory = mapOn(osAddress, size);
		PlatformAddress.memorySpy.alloc(newMemory);
		return newMemory;
	}

	public static MappedPlatformAddress mapOn(long value, long size) {
		final MappedPlatformAddress addr = new MappedPlatformAddress(value,
				size);
		return addr;
	}
	...
	public static PlatformAddress on(long value, long size) {
		final PlatformAddress addr = (value == 0) ? PlatformAddress.NULL
