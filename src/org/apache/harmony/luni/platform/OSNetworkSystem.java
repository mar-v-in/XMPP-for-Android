...
final class OSNetworkSystem implements INetworkSystem {
	...
	public boolean select(FileDescriptor[] readFDs, FileDescriptor[] writeFDs,
		...
		final int total = numReadable + numWritable;
		if (total == 0) {
			return true;
		}
		...
		final int result = selectImpl(readFDs, writeFDs, numReadable,
				numWritable, flags, timeout);
		if (result >= 0) {
			return true;
		}
	...
	private boolean validateFDs(FileDescriptor[] readFDs,
		...
		for (final FileDescriptor fd : readFDs) {
			// Also checks fd not null
			if (!fd.valid()) {
				return false;
			}
		}
		for (final FileDescriptor fd : writeFDs) {
			if (!fd.valid()) {
				return false;
			}
		}
