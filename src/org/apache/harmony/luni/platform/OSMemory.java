 ...
 * <code>long</code>. The <code>long</code> value is interpreted based on the
 * underlying platform pointer size, such that only the lowest significant
 * <code>POINTER_SIZE</code> bytes of the <code>long</code> value are used. In
 * practice this means that methods on 64-bit platforms use the full eight bytes
 * of the address parameter, and on 32-bit platforms the same methods are
 * truncated to use only the low four bytes.
 * </p>
 * <p>
 * Methods that return OS addresses define the return type to be a Java
 * <code>long</code>. If the platform pointer size is less than eight bytes the
 * OS address value is zero-extended to an eight-byte int to correspond to the
 * subsequent interpretation of that jlong as an OS address as defined above.
 * </p>
 */
final class OSMemory implements IMemorySystem {
	 ...
	 * the range that was previously allocated using <code>malloc()</code>. </p>
	 * 
	 * @param address
	 ...
	 * that was allocated using {@link #malloc(long) malloc(long)}.
	 * </p>
	 * 
	 * @param address
	...
	public double getDouble(long address, Endianness endianness) {
		...
		final long doubleBits = swap(getLong(address));
	...
	public float getFloat(long address, Endianness endianness) {
		...
		final int floatBits = swap(getInt(address));
	 ...
	 * 
	 * @return <code>false</code> for Big Endian, and <code>true</code> for
	 *         Little Endian.
	 */
	@Override
	...
	public long mmap(long fileDescriptor, long alignment, long size, int mapMode)
			throws IOException {
		// No need to check mmapImpl return as it throws IOException in error
		// cases
		final long address = mmapImpl(fileDescriptor, alignment, size, mapMode);
		return address;
	}
	...
	private int swap(int value) {
		final short left = (short) (value >> 16);
		final short right = (short) value;
		final int topEnd = swap(right) << 16;
		final int btmEnd = swap(left) & 0xFFFF;
		return topEnd | btmEnd;
	}

	private long swap(long value) {
		final int left = (int) (value >> 32);
		final int right = (int) value;
		final long topEnd = ((long) swap(right)) << 32;
		final long btmEnd = swap(left) & 0xFFFFFFFFL;
		return topEnd | btmEnd;
	}
	...
	private short swap(short value) {
		final int topEnd = value << 8;
		final int btmEnd = (value >> 8) & 0xFF;
