...
public class LdapRdnParser implements LdapParser {
	...
	private static byte[] getByteFromHexString(String val) {
		final String str = val.substring(val.indexOf("#") + 1);
		if (str.length() % 2 != 0) {
	...
	private static String getHexValues(byte[] byteArray) {
		...
		for (final byte element : byteArray) {
			sb.append(Integer.toHexString(element >> 4 & 0x0F));
			sb.append(Integer.toHexString(element & 0x0F));
		}
		return '#' + sb.toString();
	}
	...
	public List getList() throws InvalidNameException {
		try {
			...
			throw (InvalidNameException) (new InvalidNameException(
					Messages.getString("ldap.17") + name).initCause(e));
		}

		attrList = (List) listAll.get(0);
		for (final AttributeTypeAndValuePair element : attrList) {
