...
public class RelaxedDnParser extends DNParser {
	...
	protected char getEscaped() throws IOException {
		...
		default:
			/*
			 * RFC doesn't explicitly say that escaped hex pair is interpreted
			 * as UTF-8 char. It only contains an example of such DN.
			 */
	...
	public List parse() throws IOException {
		...
		final List atav = new ArrayList();
		final String charCopy = new String(chars);
		while (true) {
			if (pos == length) {
			...
			default:
				...
				if (attValue.length() > 0
					...
					if (p >= 2
							&& charCopy.substring(p - 2, p + 1).equals("\\20")) {
						attValue = attValue.substring(0, attValue.length() - 1);
					}
				}
				atav.add(new AttributeTypeAndValuePair(attType, attValue));
