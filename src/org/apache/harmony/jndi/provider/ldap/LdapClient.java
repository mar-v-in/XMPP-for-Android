...
import org.apache.harmony.javax.naming.ldap.StartTlsRequest;
...
public class LdapClient {
				...
				if (element.lock == null) {
					...
					if (element.response.getOperationIndex() == LdapASN1Constant.OP_EXTENDED_RESPONSE
							&& ((ExtendedOp) element.response.getResponseOp())
									.getExtendedRequest().getID()
									.equals(StartTlsRequest.OID)) {
						/*
		...
		public void run() {
			while (!isStopped) {
				try {
					...
					final LdapMessage response = new LdapMessage(null) {
	 ...
	 * <code>InputStream.read(byte[])</code> with byte array of zero length, the
	 * method will be blocked. Seems it's bug of ri.
	 * 
	...
	public static LdapClient newInstance(String host, int port,
		...
		final SocketFactory factory = LdapUtils
				.getSocketFactory(envmt, isLdaps);
	...
	private final Hashtable<Integer, Element> batchedSearchRequests = new Hashtable<Integer, Element>();

	/**
	 * the max time to wait server response in milli-second
	 */
	private final long MAX_WAIT_TIME = 30 * 1000;
	...
	public int addPersistentSearch(SearchOp op) throws IOException {
		final LdapMessage request = new LdapMessage(
		...
		requests.put(messageID,
				new Element(null, new LdapMessage(op.getResponse())));
		try {
	...
	public void close() {
		...
		if (requests != null) {
			for (final Element element : requests.values()) {
				if (element.lock != null) {
					synchronized (element.lock) {
						element.lock.notify();
					}
	...
	public LdapMessage doOperation(int opIndex, ASN1Encodable request,
		...
		final LdapMessage requestMsg = new LdapMessage(opIndex, request,
				controls);

		final Integer messageID = Integer.valueOf(requestMsg.getMessageId());
	...
	public void doOperationWithoutResponse(int opIndex, ASN1Encodable op,
		...
		final LdapMessage request = new LdapMessage(opIndex, op, controls);
		out.write(request.encode());
		out.flush();
	}
	...
	private LdapMessage doSearchOperation(ASN1Encodable request,
			ASN1Decodable response, Control[] controls) throws IOException {
		final int batchSize = ((SearchOp) request).getBatchSize();

		final LdapMessage requestMsg = new LdapMessage(
		...
		try {
			...
			while (responseMsg.getOperationIndex() != LdapASN1Constant.OP_SEARCH_RESULT_DONE) {
				if (size == batchSize) {
					batchedSearchRequests.put(messageID,
							requests.get(messageID));
					break;
				}
	...
	private void notifyPersistenSearchListener(Element element) {
		final PersistentSearchResult psr = (PersistentSearchResult) ((SearchOp) element.response
		...
		// notify listener
		final Control[] cs = element.response.getControls();
		if (cs != null) {
			for (final Control control : cs) {
	...
	private void notifyUnls(LdapMessage response) {
		final UnsolicitedNotificationImpl un = (UnsolicitedNotificationImpl) response
				.getResponseOp();
		for (final UnsolicitedListener listener : unls) {
