...
import org.apache.harmony.javax.naming.ldap.LdapReferralException;
...
public class ReferralExceptionImpl extends LdapReferralException {
	...
	public Context getReferralContext() throws NamingException {
		...
		final LdapUrlParser parser = LdapUtils.parserURL(referrals[index],
				false);

		final String host = parser.getHost();
		final int port = parser.getPort();

		final LdapClient client = LdapClient.newInstance(host, port, env,
				LdapUtils.isLdapsURL(referrals[index]));

		final LdapContextImpl context = new LdapContextImpl(client, env,
				targetDN);
		return context;
	...
	public Context getReferralContext(Hashtable<?, ?> h) throws NamingException {
		...
		final Hashtable<Object, Object> myEnv = (Hashtable<Object, Object>) h
				.clone();
		final LdapUrlParser parser = LdapUtils
				.parserURL(referrals[index], true);
		...
		final LdapContextImpl context = new LdapContextImpl(client, myEnv,
				targetDN);
