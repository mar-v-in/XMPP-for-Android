...
import org.apache.harmony.javax.naming.NameAlreadyBoundException;
...
import org.apache.harmony.javax.naming.directory.SchemaViolationException;
...
public class LdapSchemaContextImpl extends LdapContextImpl {
	...
	private final LdapContextImpl ldapContext;

	private final Hashtable<String, Object> schemaTable;
	...
	private final Hashtable<Name, LdapSchemaContextImpl> cachedSubSchemas = new Hashtable<Name, LdapSchemaContextImpl>();
	...
	public DirContext createSubcontext(Name name, Attributes attributes)
		...
		final Hashtable<String, Object> subSchemaTree = doLookup(
				name.getPrefix(size - 1), size - 1);
		...
		final String subSchemaType = name.getSuffix(size - 1).toString();

		if (subSchemaTree.get(subSchemaType.toLowerCase()) != null) {
			throw new NameAlreadyBoundException(subSchemaType);
		}

		final String schemaLine = SchemaParser.format(attributes);

		final ModifyOp op = new ModifyOp(ldapContext.subschemasubentry);
		final Name modifySchemaName = name.getPrefix(size - 1).addAll(rdn);
		final BasicAttribute schemaEntry = new LdapAttribute(
				new BasicAttribute(jndi2ldap(modifySchemaName.toString()),
						schemaLine), ldapContext);
	...
	public void destroySubcontext(Name name) throws NamingException {
		...
		final Object schema = subSchemaTree.get(jndi2ldap(subSchemaType));
		if (schema == null) {
			// Return silently.
			return;
		}
		...
		if (schema instanceof Hashtable) {
			final Hashtable<String, Object> table = (Hashtable<String, Object>) schema;
	...
	private Hashtable<String, Object> doLookup(Name name, int size)
		...
		for (int i = 0; i < size; i++) {
			final String key = targetDN.get(i);
			...
			if (tempValue instanceof String) {
				final Hashtable<String, Object> attributesTable = SchemaParser
	...
	private HashSet<SearchResult> doSimpleSearch(Name name,
		...
		final int size = name.size();
		final Hashtable<String, Object> subschemaTable = doLookup(name, size);

		final HashSet<SearchResult> searchResults = new HashSet<SearchResult>();
	...
	private Attributes filterAttributes(Attributes attributes,
		...
		Attribute attribute;
		final Attributes filteredAttrs = new BasicAttributes(true);
		for (final String element : attributesToReturn) {
			if (element != null) {
				attribute = attributes.get(element);
				if (attribute != null) {
	...
	public Attributes getAttributes(Name name) throws NamingException {
		...
		final Hashtable<String, Object> attributesTable = doLookup(name, size);

		final BasicAttributes schemaAttributes = new BasicAttributes(true);

		switch (level - size) {
		case 1:
			final Set<String> keyset = attributesTable.keySet();
			for (final String id : keyset) {
				if (id.equals("orig")) { //$NON-NLS-1$
					continue;
				}
				final Object value = attributesTable.get(id);
				final BasicAttribute basicAttr = new BasicAttribute(id);
		...
		case 2:
			final BasicAttribute basicAttr = new BasicAttribute("objectclass"); //$NON-NLS-1$
			final Name allName = name.addAll(rdn);
	...
	protected DirContext getClassDefinition(Attribute objectclassAttr)
			throws NamingException {
		final Hashtable<String, Object> definitionTable = new Hashtable<String, Object>();
		final Hashtable<String, Object> allDefinitionTable = (Hashtable<String, Object>) schemaTable
				.get(OBJECT_CLASSES);

		if (objectclassAttr != null) {
			final NamingEnumeration<?> ne = objectclassAttr.getAll();
			String attributeType;
			while (ne.hasMore()) {
				attributeType = ne.next().toString().toLowerCase();
				definitionTable.put(attributeType,
						allDefinitionTable.get(attributeType));
			}
		}
	...
	private boolean isMatch(Attributes schemaAttributes,
		...
		final NamingEnumeration<? extends Attribute> enumeration = matchingAttributes
				.getAll();
		...
		while (enumeration.hasMore()) {
			...
			final NamingEnumeration<?> singleEnu = matchAttribute.getAll();
	...
	public NamingEnumeration<NameClassPair> list(Name name)
		...
		final Hashtable<String, Object> tempSchema = doLookup(name, size);

		if (size == level - 1) {
			return new LdapNamingEnumeration<NameClassPair>(null, null);
		}

		final Iterator<String> keys = tempSchema.keySet().iterator();

		final List<NameClassPair> list = new ArrayList<NameClassPair>();
		while (keys.hasNext()) {
	...
	public Object lookup(Name name) throws NamingException {
		...
		final int size = targetDN.size();
		if (size == 0) {
			return this;
		}

		final Hashtable<String, Object> newSchemaTable = doLookup(name, size);
	...
	public void modifyAttributes(Name name, ModificationItem[] modificationItems)
		...
		for (final ModificationItem modificationItem : modificationItems) {
			modifyOperation = modificationItem.getModificationOp();
			modifyAttribute = modificationItem.getAttribute();
		...
		final String newSchemaLine = SchemaParser.format(attributes);
		...
		op.addModification(OperationJndi2Ldap[DirContext.REMOVE_ATTRIBUTE],
				new LdapAttribute(schemaEntry, ldapContext));
		final BasicAttribute addSchemaEntry = new LdapAttribute(
				new BasicAttribute(jndi2ldap(modifySchemaName.toString()),
						newSchemaLine), ldapContext);
		op.addModification(OperationJndi2Ldap[DirContext.ADD_ATTRIBUTE],
		...
		final Object subSchema = subSchemaTree.get(subSchemaType);
		if (subSchema instanceof String) {
			 ...
			 * changes.
			 */
			final Hashtable<String, Object> subSchemaTable = (Hashtable<String, Object>) subSchema;
			subSchemaTable.clear();
			final Hashtable<String, Object> parsedTable = SchemaParser
					.parseValue(newSchemaLine);
			final Iterator<Entry<String, Object>> it = parsedTable.entrySet()
					.iterator();
			while (it.hasNext()) {
	...
	public NamingEnumeration<SearchResult> search(Name name,
		...
		if (level - size > 1) {
			...
			for (final String string : keyset) {
				schemaName = ldap2jndi(string);
				final Name tempName = (Name) name.clone();
	...
	public NamingEnumeration<SearchResult> search(Name name, String filter,
		...
		final LdapSchemaFilter schemaFilter = new LdapSchemaFilter(filter,
				filterArgs);
		searchResults = schemaFilter.filter(searchResults);

		if (searchControls != null
				&& searchControls.getReturningAttributes() != null) {
			final String[] attributesToReturn = searchControls
			...
			if (attributesToReturn.length > 0) {
				...
				while (iterator.hasNext()) {
					...
					schemaAttributes = filterAttributes(
							searchResult.getAttributes(), attributesToReturn);
