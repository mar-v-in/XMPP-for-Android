...
public class SaslBind {
	 ...
	 */
	public LdapResult doSaslBindOperation(Hashtable env, LdapClient client,
		...
		final String host = client.getAddress();

		// Initial CallbackHandler for SaslClient
		final CallbackHandler cbh = (env
				.get("java.naming.security.sasl.callback") != null ? (CallbackHandler) env
		...
		final SaslClient saslClnt = Sasl.createSaslClient(
				new String[] { saslMech }, authorizationId, "ldap", host, env,
				cbh);
		...
		// response (if any)
		final BindOp bind = new BindOp("", "", saslMech, response);
		...
		while (!saslClnt.isComplete()
			...
			// response
			final byte[] challenge = bind.getServerSaslCreds();
	...
	private AuthMech externalValueAuthMech(Hashtable env)
		...
		if (env.get(Context.SECURITY_AUTHENTICATION) == null) {
			if (env.get(Context.SECURITY_PRINCIPAL) == null) {
				authMech = AuthMech.None;
			} else {
				authMech = AuthMech.Simple;
			}
			...
			authMech = AuthMech.SASL;
		} else {
			throw new AuthenticationNotSupportedException(
					(String) env.get(Context.SECURITY_AUTHENTICATION));
		}
	...
	private String getRealm(String creds) {
		final String[] credsProps = creds.split(",");
		for (final String credsProp : credsProps) {
			if (credsProp.startsWith("realm")) {
				System.out.println(credsProp);
				final String realm = credsProp.substring(7,
						credsProp.length() - 1);
	...
	private boolean valueSaslMech(String auth) {
		boolean flag = false;
		final String[] saslMechs = auth.trim().split(" ");

		for (final String saslMech2 : saslMechs) {
			if (saslMechs != null && saslMech2 != "") {
				if (supportedSaslMechs.contains(saslMech2)) {
					flag = true;
					saslMech = saslMech2;
					break;
				}
