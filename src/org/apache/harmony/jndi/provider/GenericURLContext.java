 ...
 * constructor calling {@link GenericURLContext#GenericURLContext(Hashtable)
 * super(environment)}.
 */
public abstract class GenericURLContext implements Context {
	...
	public void bind(Name name, Object obj) throws NamingException {
		...
		if (name.size() == 1) {
			...
			final Context context = getContinuationContext(name);
	...
	public void bind(String name, Object obj) throws NamingException {
		final ResolveResult result = getRootURLContext(name, environment);
		final Context context = (Context) result.getResolvedObj();
	...
	protected String getURLPrefix(String url) throws NamingException {
		...
		if (url.startsWith("//", index)) { //$NON-NLS-1$
			final int slashPos = url.indexOf('/', index + 2);
	...
	public void rename(Name oldName, Name newName) throws NamingException {
		...
		if (oldName.size() == 1) {
			...
			final Context context = getContinuationContext(oldName);
	...
	public void rename(String oldName, String newName) throws NamingException {
		final String oldPrefix = getURLPrefix(oldName);
		final String newPrefix = getURLPrefix(newName);
		...
		final ResolveResult result = getRootURLContext(oldName, environment);
		final Context context = (Context) result.getResolvedObj();
	 ...
	 * 
	 * @return <code>true</code> if specified URLs are equal, <code>false</code>
	 *         otherwise.
	 */
