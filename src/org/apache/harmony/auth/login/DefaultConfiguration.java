...
/**
 * Default Configuration implementation based on login configuration files. This
...
public class DefaultConfiguration extends Configuration {
	...
	public AppConfigurationEntry[] getAppConfigurationEntry(
		...
		final List<AppConfigurationEntry> list = configurations
				.get(applicationName);
	...
	private void init() {
		...
		final Map<String, List<AppConfigurationEntry>> fresh = new HashMap<String, List<AppConfigurationEntry>>();
		...
		for (final URL url : policyLocations) {
			try {
				fresh.putAll(DefaultConfigurationParser.configParser(url,
						system, fresh));
			} catch (final Exception e) {
				// TODO: log warning
				// new
				// SecurityException("Unable to load a login configuration file");
			}
		}

		// if location is not define then get a config file from user's
		// directory
		if (policyLocations.length == 0) {

			// check presence of ${user.home}/.java.login.config
			final File userLoginConfig = AccessController
			...
			try {
				...
				// ("Unable to load a login configuration file");
			}
		}
