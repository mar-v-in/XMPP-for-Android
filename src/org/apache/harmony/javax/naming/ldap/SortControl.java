...
public final class SortControl extends BasicControl {
	...
	static ASN1Sequence ASN1_SORTKEY = new ASN1Sequence(new ASN1Type[] {
		 ...
		 * 
		 * @param object
		 *            - a <code>SortKey<code> Object to encode
		 * @param values
		 *            - a Object array to return the encoded values
		 */
		@Override
		public void getValues(Object object, Object values[]) {
			final SortKey sk = (SortKey) object;
	...
	public SortControl(SortKey[] sortBy, boolean criticality)
		...
		for (final SortKey element : sortBy) {
			list.add(element);
		}
	...
	public SortControl(String[] sortBy, boolean criticality) throws IOException {
		super(OID, criticality, null);
		final ArrayList<SortKey> list = new ArrayList<SortKey>();
		for (final String element : sortBy) {
			if (element != null) {
				list.add(new SortKey(element, true, null));
			} else {
