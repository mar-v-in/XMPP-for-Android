...
public abstract class ControlFactory {
	 ...
	 * from the supplied <code>Hashtable</code> then from the resource provider
	 * files of the supplied <code>Context</code>.
	 * </p>
	 * <p>
	 ...
	 *         cannot be created, otherwise a new <code>Control</code> instance
	...
	public static Control getControlInstance(Control c, Context ctx,
		...
		for (final String element : fnames) {
			...
			final Control control = factory.getControlInstance(c);
	 ...
	 * </code> based on the supplied control. It is likely that the supplied
	 * control contains data encoded in BER format as received from an LDAP
	 * server. Returns <code>null</code> if the factory cannot create a
