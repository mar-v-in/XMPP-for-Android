...
public class LdapName implements Name {
	...
	public LdapName(String name) throws InvalidNameException {
		rdnsStr = name;
		final LdapNameParser parser = new LdapNameParser(rdnsStr);
	...
	public Name addAll(int posn, Name suffix) throws InvalidNameException {
		...
		for (final Enumeration<?> iter = suffix.getAll(); iter
				.hasMoreElements();) {
	...
	public Object clone() {
		...
		final List<Rdn> lista = new ArrayList<Rdn>();
	...
	public int compareTo(Object obj) {
		...
		final LdapName ln = (LdapName) obj;

		final Iterator<?> iter = rdns.iterator();
		final Iterator<?> iter2 = ln.rdns.iterator();

		while (iter.hasNext() && iter2.hasNext()) {
			final int c = iter.next().toString().toLowerCase()
					.compareTo(iter2.next().toString().toLowerCase());
	...
	public boolean endsWith(List<Rdn> rdns) {
		try {
			final Iterator<?> iter = rdns.iterator();
			final Iterator<?> iter2 = ((LdapName) getSuffix(this.rdns.size()
	...
	public boolean equals(Object obj) {
		...
		final Iterator<Rdn> iter = ln.rdns.iterator();
		final Iterator<Rdn> iter2 = rdns.iterator();
		while (iter.hasNext()) {
	...
	private void readObject(ObjectInputStream ois) throws IOException,
		...
		final LdapNameParser parser = new LdapNameParser(
	...
	public boolean startsWith(List<Rdn> rdns) {
		try {
			final Iterator<?> iter = rdns.iterator();
			final Iterator<?> iter2 = ((LdapName) getPrefix(rdns.size())).rdns
					.iterator();
