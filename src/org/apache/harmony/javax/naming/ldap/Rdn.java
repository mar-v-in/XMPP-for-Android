...
package org.apache.harmony.javax.naming.ldap;
...
public class Rdn implements Serializable, Comparable<Object> {
	...
	public Rdn(Attributes attrSet) throws InvalidNameException {
		...
		final NamingEnumeration<? extends Attribute> ne = attrSet.getAll();
		while (ne.hasMoreElements()) {
			final Attribute at = ne.nextElement();
			try {
				at.get();
	...
	public int compareTo(Object obj) {
		...
		final Rdn rdn = (Rdn) obj;
		String s1 = ""; //$NON-NLS-1$
		String s2 = ""; //$NON-NLS-1$

		for (final Enumeration<?> iter = toAttributes().getAll(); iter
				.hasMoreElements();) {
		...
		for (final Enumeration<?> iter = rdn.toAttributes().getAll(); iter
				.hasMoreElements();) {
	...
	private List<Attribute> convertToAttributeArrayList(Attributes attrList) {
		final LdapTypeAndValueList myList = new LdapTypeAndValueList();

		final NamingEnumeration<? extends Attribute> ne = attrList.getAll();
		try {
			while (ne.hasMoreElements()) {
				final Attribute attr = ne.nextElement();
	...
	public boolean equals(Object obj) {
		...
		final NamingEnumeration<? extends Attribute> iter1 = toAttributes()
				.getAll();
		final NamingEnumeration<? extends Attribute> iter2 = ((Rdn) obj)
				.toAttributes().getAll();

		while (iter1.hasMoreElements()) {
			final Attribute a1 = iter1.nextElement();
			final Attribute a2 = iter2.nextElement();
			...
			while (en1.hasMoreElements()) {
				final Object o1 = en1.nextElement();
				final String s1 = (o1 instanceof String) ? (String) o1
						: escapeValue(o1);

				final Object o2 = en2.nextElement();
				final String s2 = (o2 instanceof String) ? (String) o2
						: escapeValue(o2);
	...
	private void readObject(ObjectInputStream ois) throws IOException,
		...
		final String rdnString = (String) ois.readObject();
		if (rdnString == null) {
			throw new NullPointerException("rdnString "
					+ Messages.getString("ldap.00"));
		}
	...
	public Attributes toAttributes() {
		final BasicAttributes bas = new BasicAttributes(true);
		for (final Attribute attr : list) {
			final BasicAttribute ba = new BasicAttribute(attr.getID(), false);
			try {
				final NamingEnumeration nameEnum = attr.getAll();
	...
	public String toString() {
		...
		for (final Iterator<Attribute> iter = list.iterator(); iter.hasNext();) {
